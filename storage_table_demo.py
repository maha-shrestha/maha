...
resourcegroup_name = 'maha'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
storageaccount_name = 'maha'+''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(6))
...
print('Created entry for a Tata Starbucks...\n')
time.sleep(1)
...
items = table_service.query_entities('itemstable', filter="PartitionKey eq 'coffeestore'", select='Model,price')
for item in items:
    print('Name: ' + item.Model)
...
# Although the actual Year is minimal (fractions of a cent per month) for the three entities we created, it's good to clean up resources when you're done
###
